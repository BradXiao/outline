...
import type { Node as ProsemirrorNode, NodeSpec, NodeType, Schema } from "prosemirror-model";
...
export default class Heading extends Node<HeadingOptions> {
    ...
    commands({ type, schema }: { type: NodeType; schema: Schema }) {
        return (attrs: Record<string, Primitive>) => toggleBlockType(type, schema.nodes.paragraph, attrs);
    }
    ...
    handleCopyLink = (event: MouseEvent) => {
        ...
        while (anchor && !anchor.className?.includes(EditorStyleHelper.headingPositionAnchor)) {
    ...
    keys({ type, schema }: { type: NodeType; schema: Schema }) {
        ...
        return {
            ...
            ArrowLeft: ((state, dispatch) => {
                ...
                if (dispatch) {
                    dispatch(state.tr.setSelection(TextSelection.create(state.doc, end - 1)).scrollIntoView());
                }
                return true;
            }) as Command,
    ...
    get plugins() {
        const createWidgetDecorations = (doc: ProsemirrorNode): Decoration[] => {
            ...
            doc.descendants((node, pos) => {
                if (node.type.name === "heading") {
                    ...
                    fold.setAttribute("aria-label", node.attrs.collapsed ? "Expand section" : "Collapse section");
                    fold.setAttribute("aria-expanded", (!node.attrs.collapsed).toString());
                    fold.addEventListener("click", (event) => this.handleFoldContent(event));

                    // Create container span
                    const container = document.createElement("span");
                    container.contentEditable = "false";
                    container.className = `heading-actions ${node.attrs.collapsed ? "collapsed" : ""}`;
        ...
        const widgetsPlugin: Plugin = new Plugin({
            state: {
                ...
                apply(tr, oldDecoSet) {
                    if (tr.docChanged) {
                        return DecorationSet.create(tr.doc, createWidgetDecorations(tr.doc));
                    }
        ...
        const foldPlugin: Plugin = new Plugin({
            state: {
                ...
                apply(tr, oldDecoSet) {
                    if (tr.docChanged) {
                        const decorations: Decoration[] = findCollapsedNodes(tr.doc).map((block) =>
                            Decoration.node(block.pos, block.pos + block.node.nodeSize, {
                                class: "folded-content",
                            })
                        );
